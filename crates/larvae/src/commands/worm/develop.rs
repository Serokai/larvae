/*!
Running a worm from a directory, reporting what it declares, and writing its
types.

None of this needs a project or an install. A worm author has to run their
worm before it is published anywhere, and a debug session needs the same
access.
*/
...
use super::{DEFINITIONS, PACKAGE, TYPES, TYPES_FILE};
use crate::ui;
use crate::worm::Worm;

pub(super) fn run_worm(
    dir: &Path,
    file: &Path,
    config: Option<&Path>,
    out: Option<&Path>,
...
pub(super) fn info(dir: &Path) -> Result<ExitCode> {
...
pub(super) fn types(out: Option<&Path>, stdout: bool) -> Result<ExitCode> {
    if stdout {
        print!("{TYPES}");
