...
- Aliases from `coldluau.toml` and `.luaurc`, merged per key, with chain and
  cycle handling
...
- Rojo integration, mounts derived from `default.project.json` and a build
  project written to `.coldluau/build.project.json`
...
- `coldluau init`, `coldluau schema`, and `coldluau self install`, `update`
  and `uninstall`
